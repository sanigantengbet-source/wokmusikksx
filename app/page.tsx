...
export default function Home() {
  ...
  }, [latestHistoryVideoId]);

  return (
    <main className="min-h-screen pt-6 pb-32">
      <div className="flex items-center justify-between px-4 mb-4">
        <h1 className="text-2xl font-bold text-white">Beranda</h1>
        <div className="flex items-center gap-3 text-white/90">
          <Link href="/history" className="w-10 h-10 rounded-full liquid-glass-icon flex items-center justify-center text-white hover:scale-105 transition-all">
          ...
          <div className="w-10 h-10 rounded-full p-0.5 liquid-glass-icon flex items-center justify-center overflow-hidden">
      ...
      <div className="flex overflow-x-auto no-scrollbar gap-2.5 px-4 pr-12 mb-5 snap-x snap-mandatory scroll-smooth w-full">
        {pills.map((pill) => (
          <button 
            key={pill} 
            onClick={() => setActiveFilter(activeFilter === pill ? null : pill)}
            className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-semibold tracking-wide transition-all snap-center ${
              activeFilter === pill 
                ? 'bg-white text-zinc-950 font-bold shadow-lg shadow-white/10 scale-105' 
            ...
            <div className="flex overflow-x-auto no-scrollbar gap-4 px-4 snap-x snap-mandatory scroll-smooth pb-4">
              {heroTracks.map((track, i) => (
                <motion.div 
                  ...
                  className="relative w-[85vw] sm:w-[400px] shrink-0 aspect-[4/5] sm:aspect-video rounded-3xl overflow-hidden cursor-pointer group shadow-2xl snap-center hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 bg-white/5"
                ...
                >
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                    <MarqueeText text="Sounds like Raindance • Dave, Tems" className="text-sm text-white/70 pr-4" />
                    <div className="w-12 h-12 liquid-glass-accent rounded-full flex items-center justify-center shadow-lg transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                    ...
                    >
                      {chunk.map((track, j) => (
                        <div 
                          ...
                          className="relative aspect-square rounded-xl overflow-hidden cursor-pointer group hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 bg-white/5"
                          onClick={() => playTrack(track, speedDialTracks, 'similar')}
                        >
                          <SmoothImage src={getHighResImage(track.thumbnails?.[track.thumbnails.length - 1]?.url, 200)} alt={track.name} fill sizes="64px" className="object-cover" />
          ...
          {quickPicksTracks.length > 0 && (
            <div className="px-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button 
                  className="text-xs font-semibold text-white liquid-glass-pill px-4 py-2 rounded-full hover:bg-white/20 transition-all"
                      ...
                      {chunk.map((track, j) => (
                        ...
                        >
                          <div className="relative w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-white/5">
                            <SmoothImage src={getHighResImage(track.thumbnails?.[track.thumbnails.length - 1]?.url, 100)} alt={track.name} fill sizes="48px" className="object-cover" />
                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <div className="w-8 h-8 rounded-full liquid-glass-icon flex items-center justify-center text-white">
                  ...
                  return (
                    <Link href={`/artist/${artist.artistId}`} key={`artist-${artist.artistId}-${i}`}>
                      <motion.div
                        ...
                        viewport={{ once: true, amount: 0.05 }}
                        transition={{ duration: 0.4, ease: "easeOut" }}
                        className="flex flex-col items-center gap-3 cursor-pointer group shrink-0 snap-center hover:scale-105 active:scale-95 transition-all duration-200"
                      >
                        <div className="relative w-28 h-28 rounded-full overflow-hidden shadow-lg transition-transform duration-300 bg-white/5">
                          <SmoothImage src={getHighResImage(artist.thumbnails?.[artist.thumbnails.length - 1]?.url, 400)} alt={artistName} fill sizes="144px" className="object-cover" />
