...
export default function Search() {
  ...
  return (
    <main className="min-h-screen pt-6 pb-32">
      <div className="px-4 mb-4 flex items-center gap-3">
        ...
        <form onSubmit={onSubmit} className="relative flex-1">
          <input
            ...
            placeholder="Mencari lagu, artis, playlist..."
            autoFocus
            className="w-full liquid-glass text-white rounded-full py-2.5 pl-4 pr-10 focus:outline-none focus:ring-1 focus:ring-white/40 transition-all text-sm placeholder:text-white/40"
          />
          {query && (
            <button 
              type="button"
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full liquid-glass-icon flex items-center justify-center text-white/70 hover:text-white"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      <div className="flex overflow-x-auto no-scrollbar gap-2.5 mb-6 px-4 pr-12 snap-x snap-mandatory scroll-smooth w-full">
        {tabs.map((tab) => (
          <button
            key={tab}
          ...
          {suggestions.map((suggestion, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-8 h-8 rounded-full liquid-glass-icon flex items-center justify-center text-white/60">
                  <SearchIcon className="w-4 h-4" />
                </div>
                <span className="text-white text-sm font-medium">{suggestion}</span>
              </div>
              <ArrowUpLeft className="w-4 h-4 text-white/50" />
            </div>
          ))}
        </div>
      )}
          ...
          {recentSearches.map((search, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-white text-sm font-medium">{search.query}</span>
              </div>
